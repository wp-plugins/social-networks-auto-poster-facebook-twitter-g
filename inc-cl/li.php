...
if (!class_exists("nxs_snapClassLI")) { class nxs_snapClassLI {
  ...
  function showGenNTSettings($ntOpts){ global $nxs_snapThisPageUrl; $code = 'LI'; $lcode = 'li'; wp_nonce_field( 'ns'.$code, 'ns'.$code.'_wpnonce' ); 
    ...
    ?>    
    <hr/><div style="font-size: 17px;font-weight: bold; margin-bottom: 15px;">LinkedIn Settings:   <?php $cfbo = count($ntOpts); $mfbo =  1+max(array_keys($ntOpts)); ?> <?php wp_nonce_field( 'nsFB', 'nsFB_wpnonce' ); ?>
    ...
    <?php foreach ($ntOpts as $indx=>$pbo){ ?>
      ...
      <strong>Auto-publish your Posts to your <?php if(isset($pbo['liUserInfo']) && $pbo['liUserInfo']!='') echo "(".$pbo['liUserInfo'].")"; ?> LinkedIn Profile</strong>
  ...
  function showNewNTSettings($bo){ $po = array('doLI'=>'1', 'liAPIKey'=>'', 'liAPISec'=>'', 'liUserInfo'=>'', 'liAttch'=>'1', 'liOAuthToken'=>'', 'liMsgFormat'=>'New post has been published on %SITENAME%' ); $this->showNTSettings($bo, $po, true);}
  //#### Show Unit  Settings
  function showNTSettings($ii, $options, $isNew=false){  ?>
    <div id="doLI<?php echo $ii; ?>Div" <?php if ($isNew){ ?>class="clNewNTSets"<?php } ?> style="margin-left: 10px; <?php if ((isset($options['liAccessToken']) && $options['liAccessTokenSecret']!='')||$isNew) { ?>display:none;<?php } ?>">   <input type="hidden" name="apDoSLI<?php echo $ii; ?>" value="0" id="apDoSLI<?php echo $ii; ?>" />                                     
  ...
  function showEdPostNTSettings($ntOpts, $post){ $post_id = $post->ID; 
    foreach($ntOpts as $ii=>$options)  {$doLI = $options['doLI']; 
      ...
      <tr><th style="text-align:left;" colspan="2">LinkedIn AutoPoster Options (<i style="color: #005800;"><?php echo $options['liUserInfo']; ?></i>)</th><td><?php //## Only show RePost button if the post is "published"
                ...
                <tr><th scope="row" style="text-align:right; width:150px; padding-top: 5px; padding-right:10px;"><input class="nxsGrpDoChb" value="1" type="checkbox" name="SNAPincludeLI" <?php if ((int)$doLI == 1) echo "checked"; ?> /></th>
...
if (!function_exists("nxs_doPublishToLI")) { //## Second Function to Post to LI
  function nxs_doPublishToLI($postID, $options){ $blogTitle = htmlspecialchars_decode(get_bloginfo('name'), ENT_QUOTES); if ($blogTitle=='') $blogTitle = home_url(); $isPost = isset($_POST["SNAPEdit"]); 
